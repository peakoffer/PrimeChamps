  ...
  const recoverExcludedSignals = async () => {
    ...
    try {
      ...
      setMessage(payload.discoveryReused
        ...
        : `Queued ${payload.records} fresh positive cases with a $${payload.maxApifyChargeUsd.toFixed(2)} Google discovery ceiling${payload.deepDiscoveryConfigured ? " plus two focused, bounded OpenRouter/Exa source searches per athlete" : ""}. Scoring tokens and outreach mutations remain zero.`
      );
      await load();
    } catch (error) {
