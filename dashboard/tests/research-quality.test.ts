...
test("age evidence prefers exact dates and treats year-only ages conservatively", () => {
  ...
  assert.deepEqual(parseAgeEvidence("Age: 1986 • Jan 03", now), {
    age: 40,
    birthYear: 1986,
    precision: "birth_date",
  });
...
test("durable workflow code stays isolated from the Next.js request runtime", () => {
  ...
  assert.match(workflowSource, /outside the preferred.*continuing in the soft-ranking lane/);
  assert.doesNotMatch(workflowSource, /outside the target range, skipping before scoring/);
