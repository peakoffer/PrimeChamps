...
- Type checking, focused lint, 85 unit tests, and the Vercel production build pass. The live root redirects to login and the login page returns 200.
...
- The archive attribution audit quarantined 95 material signal claims that were not explicitly tied to the named athlete. The idempotent follow-up audit finds zero remaining claims in that class. Corrected global state is 1,194 safe claims and 907 sources with 100/100 point-in-time compliance; sources remain preserved for provenance.
