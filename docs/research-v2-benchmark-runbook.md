...
The live evaluation workflow now uses the same adult-evidence contract as the benchmark: a single authoritative age source is useful for safety screening but cannot qualify an adult finalist. A score can cross 80 only when two independent domains publish agreeing evidence that establishes age 21+. The rule is pinned in prompt `research-v9-corroborated-21-plus-finalist-gate` and immutable rubric/prompt artifact version 2; older single-source checkpoints do not satisfy it.
