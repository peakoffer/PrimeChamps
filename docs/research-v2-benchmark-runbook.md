...
The operator-only historical profile validators now also cover two narrowly bounded source shapes. Volleyball World evidence must be an event-scoped Beach Pro Tour player URL whose canonical link, exact player H1, DOB field, season, event start date, and dated statistics row agree; this closed Avery Poppinga's second identity/21+ source. SPEAKRJ evidence must bind the exact athlete and Instagram handle to an explicit pre-cutoff `Data Updated` field and adjacent report-header followers/uploads/engagement metrics; future projections are excluded from the stored excerpt. Eric Granado's dated 2022 snapshot closed his audience gate. Generic current profiles and projections remain ineligible.
