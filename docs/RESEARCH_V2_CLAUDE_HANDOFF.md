...
The historical scoring-and-audit benchmark and the bounded representative live-discovery proof are now **complete and passing**. The release is production-ready for owner-operated, evaluation-only research. It is not authorized for automatic outreach or automatic live-pipeline promotion.
...
- **Fresh locked release passed:** cohort `onlyfans-athlete-challenge-v2-2026-08-21-8c41eeca` was frozen as eight positive and eight negative never-scored cases. Full development replay `e4603422-5e21-4840-985e-297a77505550` completed 16/16 with benchmark runner v30 and `anthropic/claude-sonnet-5` through OpenRouter. It produced five finalists, 100% precision above 80, 100% finalist identity/21+/source/audit accuracy, zero unsupported claims, 93.75% outcome and audit-decision agreement, and cost $1.133822 under its $1.50 ceiling.
- **One-time held-out proof passed every acceptance gate:** run `8d77f5f5-8336-461d-9547-df3cd5df9519` completed the 16 locked cases once, then revealed and archived the cohort. It produced exactly eight above-80 finalists: eight true positives, zero false positives, eight true negatives, 100% precision, 100% fit/achievability/outcome agreement, 100% identity and corroborated-21+ accuracy, 100% source verification, 100% point-in-time compliance, 100% Auditor catch and finalist audit pass, and zero unsupported claims. Measured usage was 401,949 input / 19,855 output tokens and $1.002448 under the $1.50 ceiling. The cohort is archive-only and must never be tuned or reused as held out.
- **Frozen evidence stayed frozen:** commit `d4b1cb1` makes signal-recovery counts depend on real evidence-readiness gaps rather than whether an optional recovery job was previously run. Both the UI count and POST selection now reject already-ready packets, preventing unnecessary evidence changes after lock. Production reported zero locked gaps before the release began.
- **Current code/release:** runner v30 raises the conservative input-token envelope to 50,000 per case while preserving the hard dollar ceiling and replaying v29 partial checkpoints. Compound historical sport labels such as `MMA / UFC` resolve their canonical segments without accepting unrelated sports. The benchmark page retains the latest completed held-out result after its cohort becomes archive-only.
- **Representative live-discovery proof passed:** owner-run production smoke tests covered team, individual, and niche-motorsport discovery using the safe evaluation mode. Volleyball `7e72bebe-b0bd-4633-b0ba-3570f60ac304` sourced 43, scored five, and correctly returned zero. Surfing `004f6d58-c6aa-4244-a811-11e4b95ac16f` sourced 12, scored four, and returned one independently audited finalist: Sawyer Lindblad at 81. Motorcycle racing `631adb62-6fec-4484-bed2-8dd08d0fe7d1` sourced 11, reached the dedicated WorldWCR pathway, scored Ana Carrasco at 56, and correctly returned zero. All three runs completed with `quality_passed=true` and no padding.
- **Live-evaluation isolation passed:** all 66 candidate rows from those three runs have `is_test_data=true`; zero link to an athlete; zero athlete records reference the evaluation runs; and zero research notifications were created during the run window. The user-facing research form defaults to safe evaluation mode, and evaluation mode suppresses athlete, notification, draft, message, queue, and outreach writes. Normal pipeline mode remains an explicit owner action.
- **Measured live model usage:** scoring plus audit used 78,170 input / 36,739 output tokens and $0.341658 for volleyball; 54,449 / 20,397 and $0.220310 for surfing; and 6,625 / 2,576 and $0.039010 for motorcycle racing. Provider discovery/enrichment charges remain separately governed by their existing bounded budgets.
- **Measured corrections:** the live surfing run exposed a birthday-boundary conflict between an older stated age and an exact birth date. The resolver now prefers a corroborated exact birth date when agreeing sources differ by one year, and Sawyer correctly resolved as corroborated age 21 on 2026-08-20. The UI now exposes `Motorcycle Racing` separately from broad `Motorsports`, allowing the tested WorldWCR strategy to run without weakening sport identity.
...
- **Fresh-packet audit after bounded recovery:** the new eight-case signed cohort is now `7/8` freeze-ready. Lewis Buchanan, Falyn Fonoimoana, Sheena Bathory, Eric Granado, Dennis Buzukja, Mike Kimbel, and Chloé Paquet pass every identity, 21+, momentum, audience, and creator gate. Avery Poppinga remains the only blocker because no cutoff-safe audience evidence was recovered. The new dataset now contains 76 sources, 152 claims, and 121 leakage-safe claims. No scorer, Auditor, outreach, notification, or live pipeline mutation ran.
- **Ring-name evidence is explicit:** extraction v30 adds a two-source alias bridge rather than treating legal and ring names as interchangeable. Sheena's two pre-cutoff boxing articles independently connect `Sheena Bathory` to `Franciska Szabo`; a dated first-person podcast and an archived IJF profile then independently corroborate age 34. The adult gate admits legal-name evidence only after two independent alias bridges. Mere co-occurrence, one bridge, contradictory ages, and post-cutoff profiles still fail closed.
- **Current fresh challenge pool:** the production benchmark UI has 36 never-scored authoritative cases: 17 positive and 19 negative. Eight positives and ten negatives are leakage-safe and freeze-ready, so the balanced `8 fit + 8 not-fit` challenge quota is met without padding or recycling a revealed record. Seven ready positives come from the new intake and Daryn Harris is the eighth. Challenge mode may now lock the fresh held-out set while using a prior revealed `8 + 8` run only for development replay.
- **Credential and provider validation:** the prior claim that production secrets were malformed was incorrect. Vercel intentionally does not expose the values of Sensitive Environment Variables after creation, and pulled/run environments may redact or omit them; those redactions are not evidence about the deployed value. Runtime health checks and successful exact-handle Social Blade history calls proved that the deployed Apify and Social Blade credentials are usable. Dennis Buzukja, Mike Kimbel, and Chloé Paquet each gained qualifying cutoff-safe audience evidence. Avery Poppinga returned no usable historical match and remains blocked. The bounded calls ended with 57 Social Blade credits reported. Never print secret values, and continue to prefer saved Apify datasets before any paid lookup.
- **Daryn adult-evidence closure:** a strict completed-event extractor now binds the exact Tapology bout canonical URL, completed status, Boxing sport, pre-cutoff event date, exact fighter side, fighter ID, and side-scoped “Age at Fight.” It rejects upcoming/post-cutoff/wrong-sport/wrong-canonical/opponent-age cases. Daryn's January 23, 2026 completed bout reports age 26 and independently corroborates the official Florida commission DOB source. The operator capture/import cost $0 and wrote one public `adult_eligibility` claim; it did not invoke scoring or outreach.
...
- **Fresh benchmark limitation, updated:** Dylan's original authoritative set remains 44 positive / 56 negative, and both prior held-out cohorts have been revealed. The eight new signed cases remove the prior positive-count shortage, but evidence coverage—not label volume—still blocks release: the fresh quota is eight ready positives plus eight ready negatives, and the current readiness is `4 + 8`. Do not relabel, recycle revealed cases, or weaken gates to manufacture the missing positive packets.
- **Archive recovery discipline:** the durable cooldown fix recognizes bounded provider-rate-limit deferrals and blocks overlapping POST retries with `429`/`Retry-After` before any provider call. Reuse saved Apify datasets before starting a new Actor, and reserve a paid Social Blade lookup before the request so retries cannot double-spend. Scoring remains locked until at least eight fresh positives and eight fresh negatives are fully evidence-ready.
- **Validation at this cutoff:** the targeted Tapology/replay regressions and TypeScript pass. A full `npm run check` and production build must pass after the fresh-freeze UI guard correction before deployment. Historical recovery used only the explicitly bounded Social Blade calls above; no scoring, Auditor, outreach, notification, or live-pipeline action ran.
...
- Full validation at the 2026-08-20 checkpoint: typecheck, all 182 unit tests, lint with zero errors (pre-existing warnings only), and the Next.js production build pass. Post-run database isolation counts remain zero live athletes, zero candidates missing the test-data flag, and zero notifications. Static regression also prohibits draft, message, queue, and outreach writes from the evaluation workflow.
