...
- **Fresh-packet audit after import:** all eight new cases have usable point-in-time evidence, all eight pass momentum, all eight pass creator behavior, and Sheena Bathory plus Lewis Buchanan pass the audience/creator pair. Zero currently pass exact two-source identity or two-source 21+; six also lack audience evidence. Therefore `0/8` are freeze-ready and no scoring/Auditor tokens were spent. This is the correct fail-closed result, not an import failure.
- **Current fresh challenge pool:** the production benchmark UI now shows 28 never-scored authoritative cases: nine positive and 19 negative, with eight ready negatives and zero ready positives. The eight new positives plus the existing Daryn Harris case are the bounded recovery pool. Once at least eight fresh positives become evidence-ready, challenge mode may lock a fresh `8 fit + 8 not-fit` held-out set while using a prior revealed `8 + 8` run only for development replay. Do not require or manufacture 16 fresh positives.
- **Recovery start is waiting on CRM re-authentication:** the current in-app CRM session expired while starting the bounded fresh-positive recovery and redirected to Microsoft sign-in. No preparation run was inserted and no provider charge occurred. Sign back into the CRM, then use `Recover fresh positives` once; the UI caps Google/Apify discovery at $0.50 and the workflow keeps scoring tokens and outreach mutations at zero.
...
- Reach at least eight evidence-ready, never-scored positives. Eight never-scored negatives are already ready. Current state is `0 positive / 8 negative` for the challenge quota; the eight new cases plus Daryn Harris are the bounded positive recovery pool.
