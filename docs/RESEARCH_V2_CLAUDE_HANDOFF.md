...
- **Fresh-packet audit after deterministic recovery:** the new eight-case signed cohort is now `3/8` freeze-ready. Lewis Buchanan, Falyn Fonoimoana, and Sheena Bathory pass every identity, 21+, momentum, audience, and creator gate. Across the eight, seven pass two-source identity, seven pass two-source 21+, all eight pass momentum, all eight have creator behavior, and three pass the required audience-plus-creator pair. The one remaining identity/age case is Avery Poppinga; Dennis Buzukja, Eric Granado, Mike Kimbel, Chloé Paquet, and Avery still need cutoff-safe audience evidence. Current audit totals are 70 sources, 138 claims, and 107 leakage-safe claims. No scorer, Auditor, outreach, notification, or live pipeline mutation ran.
...
- **Current fresh challenge pool:** the production benchmark UI has 28 never-scored authoritative cases: nine positive and 19 negative, with eight ready negatives and three ready positives. The eight new positives plus the existing Daryn Harris case are the bounded recovery pool. Once at least eight fresh positives become evidence-ready, challenge mode may lock a fresh `8 fit + 8 not-fit` held-out set while using a prior revealed `8 + 8` run only for development replay. Do not require or manufacture 16 fresh positives.
- **Next bounded recovery:** historical audience is now the binding constraint. `npm run reuse:instagram-history -- --athlete="Name"` scans already-saved Apify Instagram profile runs and starts zero new Actors; `npm run recover:social-blade-history -- --athlete="Name" --confirm-credits=N` is the explicit one-profile paid fallback. The 2026-08-20 production env audit found `APIFY_API_KEY` malformed (11 characters, no `apify_api_` prefix) and the Social Blade client ID/token identical, so both commands fail before provider spend. Replace those production values with a real Apify token and two distinct Social Blade credentials, then redeploy. Never print their values.
...
- **Validation at this cutoff:** the full `npm run check` passes: TypeScript, lint with zero errors (pre-existing warnings only), all 180 unit tests, and the Next.js production build. No paid provider, scoring, Auditor, outreach, notification, or live-pipeline action was started for this verification.
...
- Full validation at the 2026-08-20 checkpoint: typecheck, all 180 unit tests, lint with zero errors (pre-existing warnings only), and the Next.js production build pass. Post-run database isolation counts remain zero live athletes, zero candidates missing the test-data flag, and zero notifications. Static regression also prohibits draft, message, queue, and outreach writes from the evaluation workflow.
