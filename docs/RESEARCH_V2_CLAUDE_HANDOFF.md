...
- One capped excluded-positive recovery run (`81896c04-9f22-4c30-ac68-4845abafa6ab`) spent $0.061 on a single saved Google discovery batch, processed seven records before the Internet Archive rate-limited, and added 27 safe claims. It recovered identity/momentum/creator details but zero audience claims. A zero-new-spend replay of the remaining three records on 2026-08-13 also hit the Wayback rate limit before processing any record. The workflow now has a bounded Common Crawl fallback that reuses this paid checkpoint, queries at most two pre-cutoff collections per URL, retrieves only the indexed WARC byte range, and applies the same deterministic extractor. Wayback cooldown is retained as telemetry but does not block a Common Crawl replay.
...
- Do not buy another Google/archive discovery run for the same audience gap: the measured seven-record batch produced zero audience evidence. Replay the saved checkpoint through Common Crawl only for non-audience identity, age, momentum, or creator evidence; stop if the readiness audit shows no gain.
