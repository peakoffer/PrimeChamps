...
- The corrected audit reads evidence claims in bounded batches instead of silently stopping at Supabase's default 1,000-row response cap. It currently finds 27 total evidence-ready records.
- Existing assignments are 16 development records, 16 already-revealed held-out records, and 68 fresh excluded records. Of the fresh excluded pool, one positive and 13 negatives are evidence-ready.
...
- Reach at least 16 evidence-ready excluded positives and 16 evidence-ready excluded negatives. Current state is `1 positive / 13 negative`, so the minimum gap is `15 positive / 3 negative`.
...
- Do not buy another Google/archive discovery run for the same audience gap: the measured seven-record batch produced zero audience evidence. Replay the saved checkpoint through the cutoff-safe MediaWiki/Common Crawl paths only for non-audience identity, age, momentum, or creator evidence; stop if the readiness audit shows no gain.
