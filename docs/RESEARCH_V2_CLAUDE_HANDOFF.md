...
- **Fresh-packet audit after deterministic recovery:** the new eight-case signed cohort is now `4/8` freeze-ready. Lewis Buchanan, Falyn Fonoimoana, Sheena Bathory, and Eric Granado pass every identity, 21+, momentum, audience, and creator gate. All eight now pass two-source identity, two-source 21+, momentum, and creator behavior; four pass the required audience-plus-creator pair. Dennis Buzukja, Mike Kimbel, Chloé Paquet, and Avery Poppinga still need cutoff-safe audience evidence. Current audit totals are 72 sources, 143 claims, and 112 leakage-safe claims. No scorer, Auditor, outreach, notification, or live pipeline mutation ran.
...
- **Current fresh challenge pool:** the production benchmark UI has 28 never-scored authoritative cases: nine positive and 19 negative, with eight ready negatives and four ready positives. The eight new positives plus the existing Daryn Harris case are the bounded recovery pool. Once at least eight fresh positives become evidence-ready, challenge mode may lock a fresh `8 fit + 8 not-fit` held-out set while using a prior revealed `8 + 8` run only for development replay. Do not require or manufacture 16 fresh positives.
- **Next bounded recovery:** historical audience is now the binding constraint. `npm run reuse:instagram-history -- --athlete="Name"` scans already-saved Apify Instagram profile runs and starts zero new Actors; `npm run recover:social-blade-history -- --athlete="Name" --confirm-credits=N` is the explicit one-profile paid fallback. The 2026-08-20 production env audit found `APIFY_API_KEY` malformed (11 characters, no `apify_api_` prefix) and the Social Blade client ID/token identical. Every research/evidence entry point now applies the same secret-free credential inspection before HTTP, the Connections health response reports invalid values as unusable, and the Golden benchmark page displays the exact correction while keeping paid controls disabled. Replace those production values with a real Apify token and two distinct Social Blade credentials, then redeploy. Never print their values.
...
- **Validation at this cutoff:** the full `npm run check` passes: TypeScript, lint with zero errors (pre-existing warnings only), all 183 unit tests, and the Next.js production build. No paid provider, scoring, Auditor, outreach, notification, or live-pipeline action was started for this verification.
