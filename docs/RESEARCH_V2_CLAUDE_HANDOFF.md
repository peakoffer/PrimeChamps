...
- Do not buy another Google/Wayback discovery run for the same audience gap: the measured seven-record batch produced zero audience evidence, and the saved three-record replay remains Archive-rate-limited. The saved discovery data is preserved, but leave it alone until the enforced cooldown expires and only retry if its non-audience identity/momentum evidence is still useful.
